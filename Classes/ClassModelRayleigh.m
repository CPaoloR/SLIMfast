classdef ClassModelRayleigh < matlab.mixin.Copyable
    ...
    methods
        function this = ClassModelRayleigh(parent,subpop)
            if nargin > 0
                this.Subpop = subpop;
                this.ParamNames = {...
        ...
        function [formula numParam nameParam] = build_formula(this)
            ...
            if this.Parent.Parent.IsTruncated %Mix.Model Manager
                ...
                formula = sprintf(['%s*(raylpdf(data,1/sqrt(2)*%s)/'...
                    '(raylcdf(%f,1/sqrt(2)*%s)-raylcdf(%f,1/sqrt(2)*%s)))'],...
                ...
                formula = sprintf('%s*raylpdf(data,1/sqrt(2)*%s)',...
                    this.ParamNames{1},this.ParamNames{2});
                %                 formula = [this.ParamNames{1} '*raylpdf(data,1/sqrt(2)*' ...
                %                     this.ParamNames{2} ')'];
            end %if
        ...
        function yhat = evaluate_subpopulation(this,data)
            yhat = log(this.EstValue{1})+(-(data/...
                this.EstValue{2}).^2)+log(data)-log(0.5*this.EstValue{2}^2);
            %             yhat = log(this.EstValue{1})+log(2*data)-...
            %                 2*log(this.EstValue{2})-(data/this.EstValue{2}).^2;
            %             yhat = log(this.EstValue{1}*raylpdf(data,1/sqrt(2)*this.EstValue{2}));
        end %fun
