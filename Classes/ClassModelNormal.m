classdef ClassModelNormal < handle
    ...
    properties(Hidden)
        ...
        NumParam = 3;
        ParamNames
        ParamMode = {'Bound' 'Unbound' 'Unbound'}
        UserValue = {nan nan nan};
        LB = {0.05 -inf 0};
        UB = {1 inf inf};
        InitValue
        EstValue
    ...
    methods
        function this = ClassModelNormal(parent,subpop)
            if nargin > 0
                this.Subpop = subpop;
                this.ParamNames = {...
                    ['Weight_' num2str(subpop)], ...
                    ['Mu_' num2str(subpop)], ...
                    ['Sigma_' num2str(subpop)]};
        ...
        function [formula numParam nameParam] = build_formula(this)
            ...
            if this.Parent.Parent.IsTruncated %Mix.Model Manager
                ...
                formula = sprintf(['%s*(normpdf(data,%s,%s)/'...
                    '(normcdf(%f,%s,%s)-normcdf(%f,%s,%s)))'],...
                    this.ParamNames{1},this.ParamNames{2},this.ParamNames{3},...
                    this.Parent.Parent.RightTrunc,this.ParamNames{2},this.ParamNames{3},...
                    this.Parent.Parent.LeftTrunc,this.ParamNames{2},this.ParamNames{3});
            else
                formula = sprintf('%s*normpdf(data,%s,%s)',...
                    this.ParamNames{1},this.ParamNames{2},this.ParamNames{3});
            end %if
        ...
        function yhat = evaluate_subpopulation(this,data)
            yhat = log(this.EstValue{1})+(-0.5*((data-this.EstValue{2})/...
                this.EstValue{3}).^2-log(sqrt(2*pi)*this.EstValue{3}));
            %             yhat = this.EstValue{1}*normpdf(data,this.EstValue{2},this.EstValue{3});
        end %fun
