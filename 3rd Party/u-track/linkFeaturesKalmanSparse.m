function [trackedFeatureIndx,trackedFeatureInfo,kalmanFilterInfo,...
    ...
    prevCost,hProgressbar,trackingStep)
...
numTracksWorstCase = ceil(sum(numFeatures)/10); %CPR 230415
...
for iFrame = 1 : numFrames-1
    ...
    prctDone = iFrame/(numFrames-1);
    switch trackingStep
        case 0
            update_progressbar(hProgressbar,{prctDone/2,prctDone,[]})
        case 1
            update_progressbar(hProgressbar,{prctDone/4,prctDone,[],[],[]})
        case 2
            update_progressbar(hProgressbar,{(1+prctDone)/4,[],prctDone,[],[]})
        case 3
            update_progressbar(hProgressbar,{(2+prctDone)/4,[],[],prctDone,[]})
    end %switch
    %         progressText(iFrame/(numFrames-1),text);
    %     end
